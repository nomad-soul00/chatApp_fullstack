    ...
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler} className="border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg">
        <h2 className='flex font-medium text-2xl justify-between items-center'>
          {currentState}
          {isDataSubmitted && <img src={assets.arrow_icon} onClick={()=>{setIsDataSubmitted(false)}} alt="" className='cursor-pointer w-5'  />
}
        </h2>
        ...
        {currentState === 'Sign up' && isDataSubmitted && (
          <textarea onChange={e=>{setBio(e.target.value)}} value={bio} rows={3} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2  focus:ring-indigo-500' placeholder='Provide a short bio...'></textarea>
        )}

        <button type='submit' className='py-3 bg-gradient-to-r from-purple-400 to-violet-600 text-white rounded-md cursor-pointer active:from-purple-600 active:to-violet-800'>
